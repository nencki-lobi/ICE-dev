...
``` {r}
items7 = read.table("./04/input/items-per-factor-dropouts-7.tsv", header = T, sep = "\t", encoding = "UTF-8")
rownames(items7) = items7$CODE

items11 = read.table("./04/input/items-per-factor-dropouts-11.tsv", header = T, sep = "\t", encoding = "UTF-8")
...
``` {r}
write.table(output, file="./05/output/questionnaire.tsv", row.names = F, sep='\t', fileEncoding = "UTF-8")
```