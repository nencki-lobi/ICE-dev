...
# CUSTOM FUNCTIONS
#the first tw are from Paul van der Laken (https://paulvanderlaken.com/2020/07/28/publication-ready-correlation-matrix-significance-r/#correlation_matrix), and the last one is from Mehmet Mehmetoglu.
...
### Distributions of climate emotions data {.tabset}
Let's check the assumptions for Confirmatory Factor Analysis (CFA): multi- & uni-variate normality check. We will perform formal tests: Mardia's test for multivariate skewness and kurtosis & Shapiro-Wilk for univariate normality, as well as a graphical inspection.
...
The values from the formal tests imply that we should reject the null hypotheses of uni- & multivariate normality. The plots corroborate the observation of univariate non-normality.
...
### Internal consitencies of other relevant scales
...
Cronbach's alpha for Ecocentric concern from the Environmental Attitudes Inventory was very low. Let's inspect it further with Spearman-Brown estimate, which is better for two-item scales than Cronbach's alpha coefficient (https://pubmed.ncbi.nlm.nih.gov/23089674/).
```{r, include=T, echo = F}
...
```
We accept that this scale has a very low internal consistency in this study.

But let's actually compute the Spearman-Brown estimate of internal consistency for other 2-item scales too
...
```{r, include=F}
s2.all$concern <- as.character(s2.all$cc_concern)

## subset the data for the analysis
somewhat.concerned <- s2.all %>% 
  dplyr::filter(cc_concern == 2)

very.concerned <- s2.all %>% 
  dplyr::filter(cc_concern == 3)

#merge the above dataframes
concern.discriminant <- rbind(somewhat.concerned, very.concerned)

#retain only concern and climate emotion values
concern.discriminant <- dplyr::select(concern.discriminant, c(28:36))
```
Let's start from generating a plot to get an overview of the inter-group differences.

```{r, include=F}
#change the format of the data
concern.data.plot <- gather(concern.discriminant, emotion, value, climate.anger:climate.sorrow, factor_key=TRUE)
```

```{r, include=T, echo = F}
ggplot(concern.data.plot, aes(x=emotion, y=value, fill=cc_concern)) + 
  #geom_boxplot() +
  geom_violin(trim = T) +
  ...
  scale_fill_grey(start = 1,
                  ...
                  name="Climate change concern",
                      breaks=c("2", "3"),
                      labels=c("Somewhat concerned", "Very concerned")) +
  theme_classic() +
...
The hypothesis we're testing is that climate emotions together are affected by the difference in level of climate change concern.

```{r, include=T, echo = F}
vegan::adonis2(emo.only ~ concern.discriminant$cc_concern, method = "bray",
       permutations = 999)
```
...
```{r, include=T, echo = F}
emo.dist <- vegdist(emo.only, method="bray")
dispersion <- betadisper(emo.dist, group=concern.discriminant$cc_concern)
permutest(dispersion)

#let's also plot the dispersion to make it more intuitive
plot(dispersion, hull=FALSE, ellipse=TRUE) ##sd ellipse
```

But which differences exactly are driving the overall difference between genders? We can answer this question using the approximative (Monte Carlo) Fisher-Pitman test
...
lapply(concern.discriminant[, c("climate.anger", "climate.contempt", "climate.enthusiasm", "climate.powerlessness", "climate.guilt", "climate.isolation", "climate.anxiety", "climate.sorrow")], 
       function(x) coin::pvalue(oneway_test(x ~ as.factor(cc_concern), data = concern.discriminant, 
...
```{r, include=T, echo = F}
p.adjust(c(0.00010001, 0.00010001, 0.00220022, 0.3910391, 0.00050005, 0.1214121, 0.00010001, 0.00010001), method = "BH")
```
...
#retain only concern and climate emotion values
gender.discriminant <- dplyr::select(gender.discriminant, c(28:35, 37))
...
Now, let's do a formal test, repeating the steps from the comparison between different levels of concern for climate change.
```{r, include=F}
#subset the data for the formal test
...
dicr.violin.concern <- ggplot(concern.data.plot, aes(x=emotion, y=value, fill=cc_concern)) + 
  #geom_boxplot() +
  geom_violin(trim = T) +
  geom_boxplot(width = .1, position=position_dodge(.9), outlier.shape = NA) +
  stat_boxplot(geom ='errorbar', position=position_dodge(.9)) +
  ...
  geom_signif(y_position = 5.3, xmin=c(3.9), xmax=c(4.1), annotation=".39", vjust= 0.1) +
  geom_signif(y_position = 5.3, xmin=c(4.9), xmax=c(5.1), annotation="***", vjust= 0.5) +
  geom_signif(y_position = 5.3, xmin=c(5.9), xmax=c(6.1), annotation=".14", vjust= 0.1) +
