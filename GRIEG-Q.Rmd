...
## Load items
 ...
 We load the content of the items - in Polish and in English.
 
```{r}
items =  read.table ("./input/items.tsv", header = T, sep = "\t")
```
...
We assess the reliability, by calculating Cronbach's alpha for each emotion separately:
...
for (i in 1:length(EMOTIONS)) {
  ...
  cat(sprintf("\n Reliability analysis:"))
  print(kable(out$total))
  
  cat(sprintf("\n Reliability if an item is dropped:"))
  print(kable(out$alpha.drop))
  
  cat(sprintf("\n Item statistics:"))
  print(kable(out$item.stats))
  
  cat(sprintf("\n Non missing response frequency for each item:"))
  print(kable(out$response.freq))
...
for (i in 1:nrow(rotated)) {
  ...
  criterion1 = abs(tmp[max1]) > 0.4
  criterion2 = ( abs(tmp[max1]) - abs(tmp[max2]) ) > 0.2
  
  test = rep(FALSE, length(tmp))
  test[max1] = criterion1 & criterion2
  
  thresholded[i,] = ifelse(test, tmp, '')
}
