...
We start by loading the input data. We have data from two surveys (`survey176496` and `survey381735`), both have (almost) identical structure (names of some of the columns containing timing information differ).
...
Next, we need to combine the two datasets.

We can see that only columns `14:184` contain relevant data (questionnaire items).

``` {r}
colnames(dataset1[,14:184])
```

Therefore, we combine the two datasets keeping only relevant columns. Also, let's convert the data to `dataframe` for convenience.

```{r}
dataset = rbind(dataset1[,14:184], dataset2[,14:184])
